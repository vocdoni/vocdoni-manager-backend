...
func RetrieveEntityID(ctx *httprouter.HTTPContext) (entityID []byte, signaturePubKey []byte, _ error) {
	// check public key length
	signaturePubKey, err := hex.DecodeString(dvoteutil.TrimHex(ctx.URLParam("signaturePublicKey")))
	if err != nil {
		return []byte{}, []byte{}, fmt.Errorf("cannot decode signature pubKey: %v", err)
	}
	if len(signaturePubKey) != ethereum.PubKeyLengthBytes {
		return []byte{}, []byte{}, fmt.Errorf("invalid public key: %x", signaturePubKey)
	}
