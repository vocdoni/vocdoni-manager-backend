...
func testUsersExists(t *testing.T) {
	...
	e, err := UserExists(ctx, tx, o.PublicKey)
	if err != nil {
...
func testUsersFind(t *testing.T) {
	...
	userFound, err := FindUser(ctx, tx, o.PublicKey)
	if err != nil {
		t.Error(err)
	}
...
func testUserToManySubscribedUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.PublicKey, a.PublicKey)
	queries.Assign(&c.PublicKey, a.PublicKey)
	...
	check, err := a.SubscribedUsers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if queries.Equal(v.PublicKey, b.PublicKey) {
			bFound = true
		}
		if queries.Equal(v.PublicKey, c.PublicKey) {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadSubscribedUsers(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SubscribedUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.SubscribedUsers = nil
	if err = a.L.LoadSubscribedUsers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.SubscribedUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyPushTokens(t *testing.T) {
	...
	b.UserID = a.PublicKey
	c.UserID = a.PublicKey
...
func testUserToManyAddOpSubscribedUsers(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddSubscribedUsers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.PublicKey, first.PublicKey) {
			t.Error("foreign key was wrong value", a.PublicKey, first.PublicKey)
		}
		if !queries.Equal(a.PublicKey, second.PublicKey) {
			t.Error("foreign key was wrong value", a.PublicKey, second.PublicKey)
		}

		if first.R.Subscribed != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.Subscribed != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.SubscribedUsers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.SubscribedUsers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.SubscribedUsers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpSubscribedUsers(t *testing.T) {
	var err error
	...
	err = a.SetSubscribedUsers(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetSubscribedUsers(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.PublicKey) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.PublicKey) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.PublicKey, d.PublicKey) {
		t.Error("foreign key was wrong value", a.PublicKey, d.PublicKey)
	}
	if !queries.Equal(a.PublicKey, e.PublicKey) {
		t.Error("foreign key was wrong value", a.PublicKey, e.PublicKey)
	}

	if b.R.Subscribed != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.Subscribed != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.Subscribed != &a {
	...
	if a.R.SubscribedUsers[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.SubscribedUsers[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpSubscribedUsers(t *testing.T) {
	var err error
	...
	err = a.AddSubscribedUsers(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveSubscribedUsers(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.SubscribedUsers) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.SubscribedUsers[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.SubscribedUsers[0] != &e {
...
func testUserToManyAddOpPushTokens(t *testing.T) {
	...
	for i, x := range foreignersSplitByInsertion {
		...
		if a.PublicKey != first.UserID {
			t.Error("foreign key was wrong value", a.PublicKey, first.UserID)
		}
		if a.PublicKey != second.UserID {
			t.Error("foreign key was wrong value", a.PublicKey, second.UserID)
		}
...
var (
	userDBTypes = map[string]string{`UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`, `PublicKey`: `text`, `DigestedPublicKey`: `text`}
	_           = bytes.MinRead
)
