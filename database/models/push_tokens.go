...
var (
	pushTokenAllColumns            = []string{"user_id", "created_at", "token"}
	pushTokenColumnsWithoutDefault = []string{"user_id", "token"}
	pushTokenColumnsWithDefault    = []string{"created_at"}
	pushTokenPrimaryKeyColumns     = []string{"token"}
)

type (
	// PushTokenSlice is an alias for a slice of pointers to PushToken.
	// This should generally be used opposed to []PushToken.
...
func (pushTokenL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybePushToken interface{}, mods queries.Applicator) error {
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserID == foreign.PublicKey {
...
func (o *PushToken) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	...
	values := []interface{}{related.PublicKey, o.Token}
...
func FindPushToken(ctx context.Context, exec boil.ContextExecutor, token string, selectCols ...string) (*PushToken, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"push_tokens\" where \"token\"=$1", sel,
	)
...
func (o *PushToken) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"push_tokens\" WHERE \"token\"=$1"
...
func (o *PushToken) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindPushToken(ctx, exec, o.Token)
	if err != nil {
		return err
	}
...
func PushTokenExists(ctx context.Context, exec boil.ContextExecutor, token string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"push_tokens\" where \"token\"=$1 limit 1)"
