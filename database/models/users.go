...
type User struct {
	...
	DigestedPublicKey string    `boil:"digested_public_key" json:"digested_public_key" toml:"digested_public_key" yaml:"digested_public_key"`
...
var UserColumns = struct {
	...
	CreatedAt         string
	PublicKey         string
	DigestedPublicKey string
}{
	UpdatedAt:         "updated_at",
	CreatedAt:         "created_at",
	PublicKey:         "public_key",
	DigestedPublicKey: "digested_public_key",
}
...
var UserWhere = struct {
	...
	DigestedPublicKey whereHelperstring
}{
	UpdatedAt:         whereHelpertime_Time{field: "\"users\".\"updated_at\""},
	CreatedAt:         whereHelpertime_Time{field: "\"users\".\"created_at\""},
	PublicKey:         whereHelperstring{field: "\"users\".\"public_key\""},
	DigestedPublicKey: whereHelperstring{field: "\"users\".\"digested_public_key\""},
}
...
type userR struct {
	SubscribedUsers MemberSlice    `boil:"SubscribedUsers" json:"SubscribedUsers" toml:"SubscribedUsers" yaml:"SubscribedUsers"`
...
var (
	userAllColumns            = []string{"updated_at", "created_at", "public_key", "digested_public_key"}
	userColumnsWithoutDefault = []string{"public_key", "digested_public_key"}
	userColumnsWithDefault    = []string{"updated_at", "created_at"}
	userPrimaryKeyColumns     = []string{"public_key"}
)
...
// SubscribedUsers retrieves all the member's Members with an executor via public_key column.
func (o *User) SubscribedUsers(mods ...qm.QueryMod) memberQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"members\".\"public_key\"=?", o.PublicKey),
	)
...
func (o *User) PushTokens(mods ...qm.QueryMod) pushTokenQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"push_tokens\".\"user_id\"=?", o.PublicKey),
	)
...
// LoadSubscribedUsers allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadSubscribedUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`members`),
		qm.WhereIn(`members.public_key in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.SubscribedUsers = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &memberR{}
			}
			foreign.R.Subscribed = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.PublicKey, foreign.PublicKey) {
				local.R.SubscribedUsers = append(local.R.SubscribedUsers, foreign)
				if foreign.R == nil {
					foreign.R = &memberR{}
				}
				foreign.R.Subscribed = local
				break
			}
		}
	}
...
func (userL) LoadPushTokens(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.PublicKey == foreign.UserID {
...
// AddSubscribedUsers adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.SubscribedUsers.
// Sets related.R.Subscribed appropriately.
func (o *User) AddSubscribedUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Member) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.PublicKey, o.PublicKey)
	...
	if o.R == nil {
		o.R = &userR{
			SubscribedUsers: related,
		}
	} else {
		o.R.SubscribedUsers = append(o.R.SubscribedUsers, related...)
	}
...
// SetSubscribedUsers removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Subscribed's SubscribedUsers accordingly.
// Replaces o.R.SubscribedUsers with related.
// Sets related.R.Subscribed's SubscribedUsers accordingly.
func (o *User) SetSubscribedUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Member) error {
	query := "update \"members\" set \"public_key\" = null where \"public_key\" = $1"
	values := []interface{}{o.PublicKey}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
	...
	if o.R != nil {
		for _, rel := range o.R.SubscribedUsers {
			queries.SetScanner(&rel.PublicKey, nil)
			if rel.R == nil {
				continue
			}

			rel.R.Subscribed = nil
		}

		o.R.SubscribedUsers = nil
	}
	return o.AddSubscribedUsers(ctx, exec, insert, related...)
}

// RemoveSubscribedUsers relationships from objects passed in.
// Removes related items from R.SubscribedUsers (uses pointer comparison, removal does not keep order)
// Sets related.R.Subscribed.
func (o *User) RemoveSubscribedUsers(ctx context.Context, exec boil.ContextExecutor, related ...*Member) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.PublicKey, nil)
		if rel.R != nil {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("public_key")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.SubscribedUsers {
			if rel != ri {
				continue
			}

			ln := len(o.R.SubscribedUsers)
			if ln > 1 && i < ln-1 {
				o.R.SubscribedUsers[i] = o.R.SubscribedUsers[ln-1]
			}
			o.R.SubscribedUsers = o.R.SubscribedUsers[:ln-1]
			break
		}
	}
...
func (o *User) AddPushTokens(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PushToken) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.PublicKey, rel.Token}
...
func FindUser(ctx context.Context, exec boil.ContextExecutor, publicKey string, selectCols ...string) (*User, error) {
	userObj := &User{}
	...
	query := fmt.Sprintf(
		"select %s from \"users\" where \"public_key\"=$1", sel,
	)
...
func (o *User) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"users\" WHERE \"public_key\"=$1"
...
func (o *User) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindUser(ctx, exec, o.PublicKey)
	if err != nil {
		return err
	}
...
func UserExists(ctx context.Context, exec boil.ContextExecutor, publicKey string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"users\" where \"public_key\"=$1 limit 1)"
